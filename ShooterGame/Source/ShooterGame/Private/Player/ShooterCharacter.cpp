...
{
	//AntonioM BEGIN
	if (CurrentWeapon != nullptr && CurrentWeapon->IsA(AShooterWeapon_GrenadeLauncher::StaticClass()))
	{
		CurrentWeapon->SetAlternativeTargeting(bNewTargeting);
	}
	else
	{
		bIsTargeting = bNewTargeting;

		if (!UKismetSystemLibrary::IsDedicatedServer(this) && TargetingSound)
		{
		...
		// we are the server and in control of this pawn
...
void AShooterCharacter::OnInteract()
{
	...
	if (MyPC && MyPC->IsGameInputAllowed())
	{
		...
		InteractionComponent->ServerInteract();
	}
}
// AntonioM END
